...
2. 进入`./nmap_python`，执行配置文件安装命令，levhub-key 的获取详情请见[SDK凭证获取](#sdk凭证获取)：
   ...
   # szczecin 可以替换为自己社区用户名称，nmap_python 为将要上传的工具名, --lib参数指自动生成目录结构
   $ pdm lev new szczecin.nmap_python --lib
   $ cd ./szczecin.nmap_python
   
   # username.tool/ 目录下安装 levrt 包
   $ pdm add levrt
   ```
   ...
   **以下代码中 # 之后的内容皆为代码说明，不可直接仿照形式写入或者复制粘贴到代码文件中**：
   ...
   ~~~
    ...
    用户可以在 entry( )  方法中指定工具镜像接收到参数后该如何执行命令，以及如何处理传出结果数据 [(注5)](#工具输出的结果数据将被如何处理？)：
     ...
     最后，raw 模式返回时，将会利用 Cr( ) 方法启用 `registry.cn-hangzhou.aliyuncs.com/levkit/nmap_python:v1.0` 镜像，并在容器中执行 entry( ) 方法定义的命令和输出处理，最后把数据传入 mongodb 的 `szczecin/nmap_python.alive@1.0` 空间[(注6)](#如何指定工具输出结果的存储位置？)：



3. 此时一个 无模式调用 的工具就完成了，整个 tool.py (即 nmap_python.py )文件中只有一个 alive() 函数，所以叫做无模式
