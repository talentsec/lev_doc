...
async def server_port(ip: str):
    ...
    async with Concurrent() as c :
        for domain in alive:
            print(domain)
            t = c.start(nmap_python.port_os(domain))
            task_list.append(t)
	# 接收结果
    for t in task_list:
        t_result = await t
        async for doc in t_result.all():	#获取这次并发任务的全部数据
            os_result.append(doc)
    print(os_reult)
````
