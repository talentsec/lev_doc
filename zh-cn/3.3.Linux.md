...
PDM  安装完毕，需要进行一些操作来验证PDM工具安装是否没有问题：
   ...
   * 第三个验证步骤：
   ...
   ###### 接下来配置 IDE，能够找到使 IDE 支持 PDM 的设置方式，列举了两种 IDE ：PyCharm、VSCode，建议使用VSCode
   ...
   #此外，如果你想使用环境中的工具（例如pytest），你必须将__pypackages__/<major.minor>/bin目录添加到PATH相应运行或调试配置的变量中。
   ```
   ...
   注：编译器关于PDM设置的`<major.minor>`在加入配置时，应该是自己电脑Python3的版本号，如`__pypackages__/3.10/lib`。
