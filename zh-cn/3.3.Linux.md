...
1. 在 Docker 官方下载页中找到 Linux 版本的 Docker 安装教程一步步操作即可：[安装教程](https://docs.docker.com/engine/install/)
2. 在使用潮汐开源社区的工具与编排，或自己个人本地测试工具与编排的可用性时，记得先启动 Docker。
3. 为了实现功能接入的完整性，请确认你安装的 Docker Engine 版本为 20.10.10 以上。
...
6. ###### 接下来配置 IDE，能够找到使 IDE 支持 PDM 的设置方式，例举了两种 IDE ：PyCharm、VSCode，建议使用VSCode
   ...
   #PDM 将在 .pdm.toml 中写入存储项目范围的配置，建议你在 .gitignore 中添加以下行：
   .pdm.toml
   __pypackages__/
   ```
   ...
   #此外，如果你想使用环境中的工具（例如pytest），你必须将 __pypackages__/<major.minor>/bin 目录添加到 PATH 相应运行或调试配置的变量中。
   ```
