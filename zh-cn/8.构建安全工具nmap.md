...
2. 进入`./nmap_python`，执行配置文件安装命令，levhub-token 的获取详情请见 [SDK凭证获取](#sdk凭证获取)：

   ```bash
   $ cd nmap_python
   # 配置接入潮汐系统 SDK 的凭证 levhub-token 作为全局变量，建议放入系统配置文件中，否则会在重新打开终端之后消失，每次构建新的工程项目时都需要重新导入
   # export 是 Linux 和 Mac系统的环境变量设置命令，如果您是 windows 系统，则需要使用 set 命令，即  set LEVHUB_TOKEN=your levhub-token
...
那么很有可能就是您在前面执行 `export LEVHUB_TOKEN=your levhub-token` 命令的时候没有放入系统环境变量，需要再次导入一次
