...
然后会提示选择python解释器版本和其它选项，选择 python3.10 解释器，然后一路向下
...
本步骤暂不讲解代码的具体含义，先完成剩余的步骤

### 3. 运行 Hello World

打开终端，进入工作区文件夹内，执行 `main.py`
...
所有的工具调用都是在 docker 中运行的，所以上一步代码执行之后没有任何输出，最终运行结果需要进入 docker 中查看
