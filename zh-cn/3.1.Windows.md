...
docker 在 windows 下实现的底层技术方案是通过 windows 自带的 hyper-v 虚拟机来实现的，且默认使用 nat 网络模式，这一特性使得在使用 nmap 等工具进行局域网扫描的时候，部分场景下可能会出现异常，所以本教程建议，如果电脑配置足够，那么优先建议使用 `虚拟机+Linux` 的选项，这样更加稳定，几乎不会出现异常情况。
...
##### 1.2 开启CPU支持虚拟化
 ...
 进入系统 BIOS 设置，查看 BIOS 是否开启 CPU 的虚拟化支持，如下图：
...
##### 1.4 win10 专业版、企业版、教育版
 ...
 如果是专业版、教育版或者企业版，那么不需要像家庭版那么麻烦，直接在 控制面板 -> 程序 -> 启用或关闭 windows 功能 -> 勾选 hyper-v ，然后按照提示重启即可。
 ...
 如果显示docker启动失败，报错 **"因为一个 Hyper-V 组件未运行"** ，那么就是没有启动 硬件虚拟化，请执行第一步: **1.1.1 开启CPU支持虚拟化**
...
python3 安装完毕后，请配置清华大学的 pip 源，否则在安装 python 依赖包的时候速度会很慢。
...
3. 接下来配置 IDE，能够找到使 IDE 支持 PDM 的设置方式，例举了两种 IDE ：PyCharm、VSCode，建议使用VSCode

   ###### IDE通用设置

   ```bash
   #现在大多数 IDE 中都没有 PEP 582 的内置支持或插件，你必须手动配置你的工具。
   #PDM 将在 .pdm.toml 中写入存储项目范围的配置，建议在 .gitignore 中添加以下行：
   .pdm.toml
   __pypackages__/
   ```
   ...
   #此外，如果你想使用环境中的工具（例如pytest），必须将 __pypackages__/<major.minor>/bin 目录添加到 PATH 相应运行或调试配置的变量中。
   ```
...
pdm-lev 是潮汐官方提供的 pdm 插件，能帮你自动生成符合潮汐官方要求的项目，会极大的提高你的效率
