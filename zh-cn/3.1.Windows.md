...
Docker在Windows下实现的底层技术方案是通过Windows自带的hyper-v虚拟机来实现的，且默认使用nat网络模式，这一特性使得在使用nmap等工具进行局域网扫描的时候，部分场景下可能会出现异常，所以本教程建议，如果电脑配置足够，那么优先建议使用`虚拟机+Linux`的选项，这样更加稳定，几乎不会出现异常情况。
...
3. 接下来配置 IDE，能够找到使 IDE 支持 PDM 的设置方式，列举了两种 IDE ：PyCharm、VSCode，建议使用VSCode。
   ...
   #此外，如果你想使用环境中的工具（例如pytest），必须将__pypackages__/<major.minor>/bin 目录添加到 PATH 相应运行或调试配置的变量中。
   ```

   ###### VSCode设置

   ```json
   #将以下两列加入设置的顶级字典.vscode/settings.json中，可在vscode界面settings中，选择Edit in settings.json打开：
   {
   ...
   注：编译器关于PDM设置的`<major.minor>`在加入配置时，应该是自己电脑 Python3 的版本号，如`__pypackages__/3.10/lib`。
