...
The underlying technical architecture of docker under Windows is implemented by the hyper-v virtualization technology, and the nat network mode is used by default. For that, there will be exceptions in some scenarios when using tools such as nmap for LAN scanning. Therefore, this tutorial recommends using the `virtual machine + Linux` , which is more stable and triggers almost no exceptions.
