...
### 4.	Installation result test
   ...
   * Third verification step:
   ...
   For Visual Studio Code, a project created with `pdm lev new` is configured with `.vscode/config.json`

   ###### PyCharm Settings
   Mark `__pypackages__/3.10/lib` as Sources Root. Then, select as Python interpreter a Python installation with the same `<major.minor>` version.
